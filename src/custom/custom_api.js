...
export class CustomRealtimeAPI extends RealtimeEventHandler {
    /**
     * Create a new RealtimeAPI instance
     * @param {{url?: string, apiKey?: string, dangerouslyAllowAPIKeyInBrowser?: boolean, debug?: boolean}} [settings]
     * @returns {CustomRealtimeAPI}
     */
    ...
    async connect({ model } = { model: "gpt-4o-realtime-preview-2024-10-01" }) {
        ...
        if (globalThis.document) {
            ...
            const ws = new WebSocket(
                // "wss://api.openai.com/v1/realtime?model=gpt-4o-realtime-preview-2024-10-01", // ayush changes
                this.url+"?model=gpt-4o-realtime-preview-2024-10-01", // ayush changes
                [],
                {
    ...
    send(eventName, data) {
        if (!this.isConnected()) {
            throw new Error(`CustomRealtimeAPI is not connected`);
        }
        data = data || {};
