 ...
 * @class
 */
export class CustomRealtimeClient extends RealtimeEventHandler {
    /**
    ...
    _addAPIEventHandlers() {
        ...
        const handler = (event, ...args) => {
            const { item, delta } = this.conversation.processEvent(
                event,
                ...args
            );
            return { item, delta };
    ...
    updateSession({
        ...
        tool_choice !== void 0 &&
            (this.sessionConfig.tool_choice = tool_choice);
        temperature !== void 0 &&
            (this.sessionConfig.temperature = temperature);
        max_response_output_tokens !== void 0 &&
        ...
        if (content.length) {
            for (const c of content) {
                if (c.type === "input_audio") {
                    if (
                        c.audio instanceof ArrayBuffer ||
                        c.audio instanceof Int16Array
                    ) {
    ...
    cancelResponse(id, sampleCount = 0) {
        if (!id) {
            ...
            this.realtime.send("response.cancel");
            const audioIndex = item.content.findIndex(
                (c) => c.type === "audio"
            );
            if (audioIndex === -1) {
